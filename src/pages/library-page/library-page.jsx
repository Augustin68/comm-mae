...
import { LibraryCategories, libraryBooks } from './library-books';
    ...
    return (
        <div className="flex bg-skin-primary h-screen p-5 pl-32 max-w-full justify-center items-center">
            <div className="w-11/12 h-3/4 flex lg:flex-row flex-col justify-between items-center 2xl:gap-8 xl:gap-5 gap-3 max-lg:overflow-auto">
                {
                     Object.keys(LibraryCategories).map((categoryKey, index) => (
                        <Link to={`/library/${categoryKey}`} key={index}
                        ...
                        >
                                <p className={`${libraryBooks.find(book => book.id === LibraryCategories[categoryKey])?.customClass} lg:writing-vertical-rl lg:m-auto lg:rotate-180 text-skin-primary xl:text-3xl text-xl font-bold`}>{LibraryCategories[categoryKey]}</p>
                                <div>
